...
def plot_k_anonymity_metrics(data, output_dir):
    ...
    g = sns.catplot(
        data=df, kind="bar",
        x="Table", y="Value", hue="Dataset", col="Metric",
    ...
    plt.suptitle("K-Anonymity Metrics Comparison - Log Scale (Lower is Better)", y=1.05)
    plt.tight_layout()
...
def plot_group_size_distribution(data, output_dir):
    ...
    fig, axes = plt.subplots(len(data["comparisons"]), 2, figsize=(15, 6*len(data["comparisons"])))
    ...
    for i, comp in enumerate(data["comparisons"]):
        ...
        ax.set_yscale('log')
        ax.set_title(f"Group Size Distribution - {table_name} (Log Scale)")
        ...
        ax.set_title(f"Group Size Distribution - {table_name} (Percentage)")
        ax.set_xlabel("Group Size")
...
def plot_l_diversity_metrics(data, output_dir):
    ...
    if not use_log:
        ...
        plt.title("L-Diversity: Problematic Groups Percentage - Log Scale\n(Lower is Better)")
...
def plot_summary_comparison(data, output_dir):
    """Create a summary visual comparison between original and synthetic data privacy."""
    summary_data = []
    
    for comp in data["comparisons"]:
        table_name = TABLE_NAMES.get(comp["table_name"], comp["table_name"])
        
        # Extract key metrics that indicate privacy
        orig_unique_pct = comp["dataset1_results"]["high_risk_percentage"]
        synth_unique_pct = comp["dataset2_results"]["high_risk_percentage"]
        ...
        summary_data.append({
            ...
            "Improvement": orig_unique_pct - synth_unique_pct
        })
        
        # Add l-diversity metrics for each sensitive attribute
        for attr, orig_values in comp["dataset1_results"]["l_diversity"].items():
            if attr in comp["dataset2_results"]["l_diversity"]:
                synth_values = comp["dataset2_results"]["l_diversity"][attr]
                
                # Get the problematic groups percentage
                orig_prob = orig_values["problematic_groups_percentage"]
                synth_prob = synth_values["problematic_groups_percentage"]
                
                # Add to summary data
                attr_name = attr.replace("_diagnosis", "").replace("_procedure_code", "")
                summary_data.append({
                    "Table": table_name,
                    "Metric": f"{attr_name} Problematic Groups (%)",
                    "Original": orig_prob,
                    "Synthetic": synth_prob,
                    "Improvement": orig_prob - synth_prob
                })
    ...
    # Draw the privacy improvement as a heatmap
    improvements = df.pivot(index="Metric", columns="Table", values="Improvement")
    
    # Determine the colormap based on whether higher or lower is better
    # For these metrics, positive improvement means synthetic is better
    # (except for attributes where negative values indicate synthetic is better)
    cmap = sns.diverging_palette(220, 10, as_cmap=True)
    
    # Plot the heatmap
    sns.heatmap(improvements, annot=True, cmap=cmap, center=0, 
                fmt=".2f", linewidths=.5, cbar_kws={"label": "Improvement (Original - Synthetic)"})
    
    plt.title("Privacy Protection Improvement in Synthetic Data\n(Positive values indicate better privacy in synthetic data)")
    plt.tight_layout()
    plt.savefig(output_dir / 'privacy_improvement_summary.png', dpi=300, bbox_inches='tight')
    plt.close()
    
    # Create a side-by-side comparison
    plt.figure(figsize=(14, 10))
    
    # Set up the data for side-by-side comparison
    comparison_data = []
    ...
    ax = sns.catplot(
        ...
        x="Metric", y="Value", hue="Dataset", col="Table",
        palette="dark", alpha=.8, height=6, aspect=1.5
    )
    ...
    for ax_i in ax.axes.flat:
        ...
        ax_i.set_ylabel("Percentage (Log Scale)")
        ...
        for i, bar in enumerate(ax_i.patches):
            ...
            if height > 0:  # Only label if the value is visible
                ax_i.text(
                    bar.get_x() + bar.get_width()/2, height * 1.05,
                    f"{height:.2f}%", ha='center', va='bottom',
                    color='black', fontsize=8, rotation=45
                )
    
    plt.suptitle("Privacy Metrics Comparison (Lower is Better)", y=1.05)
    plt.tight_layout()
    plt.savefig(output_dir / 'privacy_metrics_comparison.png', dpi=300, bbox_inches='tight')
    plt.close()
...
def main():
    ...
    # Generate plots
    plot_k_anonymity_metrics(data, output_dir)
    plot_group_size_distribution(data, output_dir)
    plot_l_diversity_metrics(data, output_dir)
    plot_t_closeness_metrics(data, output_dir)
